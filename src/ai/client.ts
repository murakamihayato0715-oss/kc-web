...
export async function suggestFleet(
  ...
  const prompt = `あなたは艦これ（艦隊これくしょん）の熟練提督です。
...
export async function chatWithAi(
  ...
  const systemInstructionText = `あなたは艦これ（艦隊これくしょん）の熟練提督であり、ユーザーの頼れるアドバイザー（AI提督）です。
ユーザー（提督）からの質問や編成相談に対して、親身になって回答してください。
