...
3. **補強増設（拡張スロット）の積極装着ルール**:
   - 手持ち艦娘リストに \`[補強増設あり]\` と表記されている艦娘には、所持リストにあるタービン、測距儀、増設バルジ、見張員、機銃、戦闘糧食等の対応装備を「補強増設: 装備名★+X」として絶対に空欄にせず積極的にセットしてください。
4. **基地航空隊の専用装備（陸攻・陸戦・陸偵）配置規則**:
   - 基地航空隊の装備スロットには、艦載機だけでなく所持リストにある「一式陸攻」「陸攻」「陸上偵察機」「九六式陸攻」などの基地専用機・陸上攻撃機・陸戦を優先して配置してください。
5. **海域ごとの出撃可能基地数の遵守（特に6-5等）**:
   - 6-5海域では第1・第2基地航空隊の2部隊のみが出撃可能です（第3基地は防空専用）。6-5では第3基地航空隊を出撃編成せず空（または防空）とし、第1・第2基地航空隊に陸攻・陸戦・陸偵を集中配備してください。
6. **海域ID (mapId) の明記**:
   - リクエストされた海域（6-5なら65、5-5なら55）の数値IDを、返却JSONの \`mapId\` プロパティに必ず設定してください。
`;
