...
2. **上位性能・フィットボーナス装備の積極分散配置（同一装備の安易な固め禁止）**:
   - 同じ装備（12.7cm連装砲D型改二や4連装酸素魚雷など）を艦隊全体で機械的に連続重複して固定化してはいけません。
   - 所持リストにある「試製61cm六連装(酸素)魚雷」「61cm五連装(酸素)魚雷」「12.7cm連装砲B型改四(コングラ)」「SG レーダー」「152mm/55 三連装速射砲」「水雷戦隊見張員」など、より高い単体性能やフィットボーナス・シナジー効果を持つ上位装備を各艦娘の特性に合わせて最優先で分散アセンブルしてください。
