...
describe('Sortie Simulation Settings Tests', () => {
  ...
  describe('sim-worker logic simulation', () => {
    it('should correctly count consumed Damecons and resupply them from the global pool in carry-over mode', () => {
      // Mock the globals
      const CARRYOVERHP = true;
      const CARRYOVERMORALE = true;
      ...
      const ship: any = {
        ...
        reset(notHP: boolean, notMorale: boolean) {
          // Wrapped Ship.prototype.reset logic
          if (this.repairsOrig) {
            const count42Orig = this.repairsOrig.filter((id: number) => id == 42).length;
            const count42 = this.repairs ? this.repairs.filter((id: number) => id == 42).length : 0;
            const cons42 = count42Orig - count42; // 2 - 1 = 1
            TEMP_DAMECON_USED += cons42;
            TOTAL_DAMECON_USED += cons42;

            const count43Orig = this.repairsOrig.filter((id: number) => id == 43).length;
            const count43 = this.repairs ? this.repairs.filter((id: number) => id == 43).length : 0;
            const cons43 = count43Orig - count43; // 1 - 0 = 1
            TEMP_GODDESS_USED += cons43;
            TOTAL_GODDESS_USED += cons43;

            if (CARRYOVERHP) {
              const resupply42 = Math.min(cons42, GLOBAL_DAMECON_POOL);
              GLOBAL_DAMECON_POOL -= resupply42;

              const resupply43 = Math.min(cons43, GLOBAL_GODDESS_POOL);
              GLOBAL_GODDESS_POOL -= resupply43;

              let deficit42 = cons42 - resupply42;
              let deficit43 = cons43 - resupply43;
              const nextRepairs = [];
              for (const id of this.repairsOrig) {
  ...
  describe('canContinue submarine decoy override tests', () => {
    it('should allow advancing on submarine decoy Taiha when decoy is active, but retreat when decoy is inactive', () => {
      ...
      const resultInactive = runWrappedCanContinue(ships, null, false);
      equal(resultInactive, false);
      equal(ships[1].HP, 4); // should still be 4

      // Test 2: Decoy is active -> should advance (return true)
      const resultActive = runWrappedCanContinue(ships, null, true);
      ...
      const resultWithDD = runWrappedCanContinue(shipsWithTaihaDD, null, true);
